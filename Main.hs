...
jsonBool = f <$> (stringP "true" <|> stringP "True" <|> stringP "false" <|> stringP "False") 
...
jsonObject = JsonObject <$> (charP '{' *> ws *> sepBy (ws *> charP ',' <* ws) pair <* ws <* charP '}')
    where pair = (\key _ value-> (key, value)) <$> stringLiteral 
                <*> (ws *> charP ':' *> ws) <*> 
                jsonValue
